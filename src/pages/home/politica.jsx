...
const Politica = () => {
  ...
  useEffect(() => {
    fetch("https://apidemo.cartavirtual.shop/api/politicas")
      .then((res) => res.json())
      .then((data) => setData(data))
      .catch(() => setError("No se pudieron cargar las políticas de privacidad."));
  }, []);
  ...
  useEffect(() => {
    fetch("https://apidemo.cartavirtual.shop/api/empresa")
      ...
      .then((empresaData) => {
        ...
        document.title = `${empresaData.nombre} / Política`;
      })
  ...
  return (
    <div className={`w-full min-h-screen transition-colors duration-300 ${darkMode ? "bg-[#262837] text-white" : "bg-gray-100 text-[#111]"}`}>
      ...
      {/* Contenedor principal con margen en escritorio */}
      <div className="lg:ml-28 lg:px-8 pb-24 lg:pb-10">
        <div className="p-4 md:p-6">

          {/* Título con flecha (solo en pantallas pequeñas) */}
          <div className="flex items-center gap-3 mb-6">
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-[#F0320C]">
          ...
          <footer className="py-6 text-center text-sm mt-10">
            ...
            <div className="flex justify-center gap-4 mt-2">
              <a href="/terminos" className="hover:text-amber-500 transition-colors">Términos de Servicio</a>
              <span>•</span>
              <a href="/politicas" className="text-amber-500 font-medium">Política de Privacidad</a>
            </div>
          </footer>
        </div>
      </div>
